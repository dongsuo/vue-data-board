  ...
  <div class="dashboard-wrapper">
    <div class="tool-bar">
      ...
      <div v-show="mode === 'edit'">
        ...
        <el-button type="primary" size="mini" @click="handleLinkChart">添加图表</el-button>
      </div>
    </div>
    <grid-layout
        ...
        >
          <div slot="header" class="operation-bar">
            ...
            <div>
              ...
              />
              <el-tooltip
                :content="getChartItem(item.i).desc"
                class="item"
                effect="dark"
                placement="top-end"
              >
    ...
    >
      ...
      <div>
        <el-link type="info" :underline="false">
          <router-link to="/chartpanel/create">Dashboard Is Empty，Go Create Your First Chart!</router-link>
        </el-link>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    handleShare() {
      ...
      const link = `${process.env.VUE_APP_DEPLOYMENT_URL}/vue-data-board#/fullscreendb/${this.dashboard.objectId}`;
      this.$msgbox({
        title: "分享链接",
        message: h("p", null, [
          h(
    ...
    isExisted(chart) {
      return (
        this.charts.findIndex(item => item.objectId === chart.chart_id) >= 0
      );
    },
    handleEdit(chart) {
    ...
    handleDelete(chart) {
      this.$confirm(
        "该操作将从该 Dashboard 中删除该图表，并不会删除原图表，确认继续?",
        "提示",
        {
